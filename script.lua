...
CreateToggleSwitch(rDesync, Config.DesyncVisualizer, function(ns, set)
    set(ns)
    if _G.SetDesync then _G.SetDesync(ns) else Config.DesyncVisualizer = ns; SaveConfig() end
...
local rDesyncFreeze = CreateRow("Freeze Server Pos on Desync")
CreateToggleSwitch(rDesyncFreeze, Config.DesyncFreezePos, function(ns, set)
    set(ns); Config.DesyncFreezePos = ns; SaveConfig()
    ShowNotification("DESYNC FREEZE POS", ns and "ENABLED" or "DISABLED")
end)
...
task.spawn(function()
    -- Marquer ce joueur comme utilisateur du script
    pcall(function() LocalPlayer:SetAttribute("SpykeUser", true) end)
    ...
    local function makePlayerBillboard(player)
        ...
        -- Affiche "Spyke [NOM]" si le joueur a le script, sinon juste le nom
        local ok, isSpyke = pcall(function() return player:GetAttribute("SpykeUser") end)
        nameLbl.Text = (ok and isSpyke) and ("Spyke ["..player.Name.."]") or player.Name
        return bb, nameLbl
    end
    ...
    local function createOrRefresh(player)
        ...
        if not entry or not entry.bb or not entry.bb.Parent then
            ...
            -- Mettre à jour le label si l'attribut SpykeUser change
            pcall(function()
                player:GetAttributeChangedSignal("SpykeUser"):Connect(function()
                    local isSpyke = player:GetAttribute("SpykeUser")
                    nameLbl.Text = isSpyke and ("Spyke ["..player.Name.."]") or player.Name
                end)
            end)
        else
            if entry.bb.Adornee ~= hrp then entry.bb.Adornee = hrp; entry.bb.Parent = hrp end
            -- Rafraîchir le texte au cas où l'attribut a changé
            local ok2, isSpyke2 = pcall(function() return player:GetAttribute("SpykeUser") end)
            entry.nameLbl.Text = (ok2 and isSpyke2) and ("Spyke ["..player.Name.."]") or player.Name
        end
    end
...
task.spawn(function()
    ...
    local frozenServerPos = nil  -- position figée quand on active la desync
    local cframeConn
    ...
    -- Fonction pour activer/désactiver la desync et figer la server pos
    local function setDesync(enabled)
        Config.DesyncVisualizer = enabled
        if enabled then
            -- Figer la server pos à la position actuelle du joueur
            local char = LocalPlayer.Character
            local hrp = char and char:FindFirstChild("HumanoidRootPart")
            if hrp then
                frozenServerPos = hrp.Position
                serverPos = frozenServerPos
            end
        else
            frozenServerPos = nil
        end
    ...
    RunService.Heartbeat:Connect(function()
        ...
        if Config.DesyncFreezePos and frozenServerPos then
            -- Mode freeze : la server pos reste figée à l'endroit d'activation
            serverPos = frozenServerPos
        else
    ...
    UserInputService.InputBegan:Connect(function(inp, gp)
        ...
        if Config.DesyncKey ~= "" and inp.KeyCode == Enum.KeyCode[Config.DesyncKey] then
            setDesync(not Config.DesyncVisualizer)
            ShowNotification("DESYNC", Config.DesyncVisualizer and "ENABLED" or "DISABLED")
        end
    end)

    -- Auto Desync on Steal : surveille le message "You stole"
    task.spawn(function()
        while true do
            task.wait(0.3)
            if not Config.AutoDesyncOnSteal then continue end
            for _, gui in ipairs(PlayerGui:GetDescendants()) do
                local txt = (gui:IsA("TextLabel") or gui:IsA("TextButton")) and gui.Text
                if txt and string.find(txt, "You stole") then
                    if not Config.DesyncVisualizer then
                        setDesync(true)
                        ShowNotification("AUTO DESYNC", "DESYNC ACTIVÉ (STEAL)")
                    end
                    break
                end
            end
