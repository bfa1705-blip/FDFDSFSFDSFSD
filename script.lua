...
task.spawn(function()
    ...
    local function executeInstantSteal(prompt)
        ...
        if SharedState.OnStealFired then SharedState.OnStealFired() end
    end
...
UserInputService.InputBegan:Connect(function(input, processed)
    ...
    if Config.DesyncKey ~= "" and Enum.KeyCode[Config.DesyncKey] and input.KeyCode == Enum.KeyCode[Config.DesyncKey] then
...
local rAutoDesync = CreateRow("Auto Desync On Steal")
CreateToggleSwitch(rAutoDesync, Config.AutoDesyncOnSteal, function(ns, set)
    set(ns); Config.AutoDesyncOnSteal = ns; SaveConfig()
    ShowNotification("AUTO DESYNC ON STEAL", ns and "ENABLED" or "DISABLED")
end)
...
task.spawn(function()
    ...
    -- serverPos = position "gelée" côté serveur (ne bouge plus quand desync actif)
    local serverPos = Vector3.new(0,0,0)
    local desyncFrozen = false  -- true = serverPos figée à l'endroit où on a activé le desync
    ...
    -- cAtt est recréé à chaque nouveau HRP pour éviter l'erreur de re-parenting
    local cAtt = nil
    ...
    beam.Attachment1 = sAtt
    beam.Enabled = false
    beam.Parent = spp

    local function refreshCatt(hrp)
        if cAtt and cAtt.Parent then cAtt:Destroy() end
        cAtt = Instance.new("Attachment", hrp)
        beam.Attachment0 = cAtt
        currentHrp = hrp
    end
    ...
    -- Exposé pour que l'auto-desync on steal puisse figer la serverPos
    _G.FreezeDesyncPos = function()
        local char = LocalPlayer.Character
        local hrp = char and char:FindFirstChild("HumanoidRootPart")
        if hrp then
    ...
    _G.UnfreezeDesyncPos = function()
        desyncFrozen = false
    end

    local lastDesyncKey = Config.DesyncKey
    RunService.Heartbeat:Connect(function()
        ...
        -- Recréer cAtt si le HRP a changé (respawn)
        if hrp ~= currentHrp then
            refreshCatt(hrp)
            serverPos = hrp.Position
            desyncFrozen = false
        end

        -- Mettre à jour serverPos seulement si pas figée
        ...
        local col = desyncFrozen and Color3.fromRGB(255, 80, 80) or Theme.Accent1
        spp.Color = col
        beam.Color = ColorSequence.new(col)
        tl.TextColor3 = col
        local dist = (serverPos - hrp.Position).Magnitude
        tl.Text = string.format("%sSERVER POS\n%.1f studs", desyncFrozen and "[FROZEN] " or "", dist)
    end)
...
do
    local desyncCooldown = 0
    local DESYNC_COOLDOWN = 0.5

    local function triggerDesync()
        if not Config.AutoDesyncOnSteal then return end
        local now = os.clock()
        if now - desyncCooldown < DESYNC_COOLDOWN then return end
        desyncCooldown = now

        -- Fige la serverPos du visualizer à la position actuelle du joueur
        if _G.FreezeDesyncPos then
            _G.FreezeDesyncPos()
        end

        -- Spoof des packets raknet si disponible
        if raknet and raknet.add_send_hook then
            local hookActive = true
            local hookConn = raknet.add_send_hook(function(packet)
            ...
            task.delay(0.5, function()
                hookActive = false
                pcall(function() if hookConn and hookConn.Disconnect then hookConn:Disconnect() end end)
                -- Dégèle la serverPos après le desync
                if _G.UnfreezeDesyncPos then _G.UnfreezeDesyncPos() end
            end)
        else
            -- Sans raknet, dégèle après un court délai
            task.delay(0.5, function()
                if _G.UnfreezeDesyncPos then _G.UnfreezeDesyncPos() end
            end)
        end
    ...
    SharedState.OnStealFired = triggerDesync
end

-- ── SPYKE [USER] BILLBOARD ───────────────────────────────────────────────
task.spawn(function()
    local TAG_NAME = "SpykeBillboard"
    local billboards = {}
    ...
    local function hookPlayer(player)
        if player == LocalPlayer then return end
        -- Wait for character and create billboard
    ...
    -- Keep billboards alive (re-create if destroyed)
    RunService.Heartbeat:Connect(function()
        for _, p in ipairs(Players:GetPlayers()) do
