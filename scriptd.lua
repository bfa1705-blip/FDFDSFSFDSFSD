...
local function startAntiRagdoll(mode)
    ...
    if Config.AntiRagdollV2 then
        pcall(function() stopAntiRagdollV2() end)
    end
...
local rShowBestBrainrot = CreateRow("Show Best Brainrot HUD")
CreateToggleSwitch(rShowBestBrainrot, Config.ShowBestBrainrotHUD, function(ns, set)
    set(ns); Config.ShowBestBrainrotHUD = ns; SaveConfig()
    local hudGui = PlayerGui:FindFirstChild("XiBestBrainrotHUD")
    if hudGui then hudGui.Enabled = ns end
    ShowNotification("BEST BRAINROT HUD", ns and "ENABLED" or "DISABLED")
end)
...
task.spawn(function()
    ...
    hudGui.Name = "XiBestBrainrotHUD"
    hudGui.ResetOnSpawn = false
    hudGui.DisplayOrder = 10
    hudGui.Enabled = Config.ShowBestBrainrotHUD
    hudGui.Parent = PlayerGui

    local hudFrame = Instance.new("Frame")
    hudFrame.Size = UDim2.new(0, 280, 0, 70)
    hudFrame.Position = UDim2.new(0.5, -140, 0, 10)
    hudFrame.BackgroundColor3 = Color3.fromRGB(8, 10, 18)
    ...
    hudStroke.Color = Theme.Accent2
    hudStroke.Thickness = 1.2
    hudStroke.Transparency = 0.4

    local accentBar = Instance.new("Frame", hudFrame)
    ...
    local headerLbl = Instance.new("TextLabel", hudFrame)
    headerLbl.Size = UDim2.new(1, -20, 0, 14)
    headerLbl.Position = UDim2.new(0, 16, 0, 6)
    headerLbl.BackgroundTransparency = 1
    headerLbl.Text = "BEST BRAINROT"
    headerLbl.Font = Enum.Font.GothamBlack
    headerLbl.TextSize = 10
    headerLbl.TextColor3 = Theme.Accent1
    headerLbl.TextXAlignment = Enum.TextXAlignment.Left

    local nameLbl = Instance.new("TextLabel", hudFrame)
    nameLbl.Size = UDim2.new(1, -20, 0, 20)
    nameLbl.Position = UDim2.new(0, 16, 0, 22)
    nameLbl.BackgroundTransparency = 1
    nameLbl.Text = "---"
    ...
    local infoLbl = Instance.new("TextLabel", hudFrame)
    infoLbl.Size = UDim2.new(1, -20, 0, 16)
    infoLbl.Position = UDim2.new(0, 16, 0, 44)
    infoLbl.BackgroundTransparency = 1
    infoLbl.Text = "---"
    infoLbl.Font = Enum.Font.GothamMedium
    infoLbl.TextSize = 11
    infoLbl.TextColor3 = Theme.TextSecondary
    ...
    -- Track when each uid first appeared
    local firstSeenTimes = {}

    task.spawn(function()
        while true do
            task.wait(0.5)
            if not Config.ShowBestBrainrotHUD then continue end

            local cache = SharedState.AllAnimalsCache
            if not cache or #cache == 0 then
                nameLbl.Text = "No brainrots found"
                infoLbl.Text = "---"
                continue
            end

            -- cache is sorted highest genValue first, owner field is already the username string
            -- skip animals whose owner matches the local player
            local myName = LocalPlayer.Name
            local myDisplayName = LocalPlayer.DisplayName
            local best = nil
            for _, a in ipairs(cache) do
                local ownerStr = tostring(a.owner or "")
                if ownerStr ~= myName and ownerStr ~= myDisplayName then
                    best = a
                    break
                end
            end
            ...
            -- Track first seen time per uid
            if not firstSeenTimes[best.uid] then
                firstSeenTimes[best.uid] = os.clock()
            end
            local elapsed = math.floor(os.clock() - firstSeenTimes[best.uid])

            local hasMut = best.mutation and best.mutation ~= "None" and best.mutation ~= "N/A"
            local color = hasMut and (MUT_COLORS[best.mutation] or Color3.fromRGB(200, 100, 255)) or Color3.fromRGB(0, 255, 150)

            nameLbl.TextColor3 = color
            local mutStr = hasMut and ("[" .. best.mutation .. "] ") or ""
            nameLbl.Text = mutStr .. (best.name or "???") .. "  " .. (best.genText or "")
            infoLbl.Text = "Base: " .. (best.owner or "?") .. "  |  " .. elapsed .. "s in base"
